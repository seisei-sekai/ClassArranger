...
**Purpose:** 从零开始将 ClassArranger 项目部署到 GCP Cloud Run 的完整详细指南
...
# 设置默认区域
gcloud config set compute/region asia-northeast1  # 东京区域
```
...
4. 选择位置：**asia-northeast1 (东京)** 或离你最近的区域
5. 点击 **"启用"**
...
# 删除服务
gcloud run services delete frontend --region asia-northeast1
gcloud run services delete backend --region asia-northeast1
