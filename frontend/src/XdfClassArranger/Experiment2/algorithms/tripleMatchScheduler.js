...
export class TripleMatchScheduler {
    ...
    return Object.values(teacherAvailability)
      .filter(info => {
        ...
        if (Array.isArray(teacher.campus)) {
          if (!teacher.campus.includes(campus)) {
            return false;
          }
        } else if (teacher.campus && teacher.campus !== campus) {
          return false;
        }
  ...
  updateAvailability(availabilityMap, resourceId, assignedSlot) {
    ...
    resourceInfo.availableSlots = resourceInfo.availableSlots.map(slot => {
      ...
      return splitSlots;
    }).flat().filter(slot => slot.endSlot > slot.startSlot);
